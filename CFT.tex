...
对称性是研究宇宙自然规律的一个基本出发点，通常来说一个物理系统的对称性越高，它的物理机制越简单。而在场论中，对称性意味着在一个对称变换下物理系统的作用量不变，相应的运动方程也不变，{\heiti 可以证明，共形对称性是具有非平庸关联函数的理论所能拥有的最大对称性。}而研究具有共形不变性的经典或量子场论，就是所谓的{\heiti 共形场论（Conformal Field Theory）。}
...
不难验证它满足这样的对易关系
\begin{align}
				\left[ J_{ab},J_{cd} \right]=i \left( g_{ad}J_{bc}+g_{bc}J_{ad}-g_{ac}J_{bd}-g_{bd}J_{ac}\right)
\end{align}
